...
defmodule Fizzbuzz do
  ...
  def is_fizz_buzz(value, fizz, buzz) do
    fizz_remaining = rem(value, fizz)
    buzz_remaining = rem(value, buzz)

    case {fizz_remaining, buzz_remaining} do
      {0, 0} -> "FB"
      {0, _} -> "F"
      {_, 0} -> "B"
      {_, _} -> value
  ...
  def get_serie(fizz_divider, buzz_divider, limit) do
    Enum.map(
      1..limit,
      &is_fizz_buzz(&1, fizz_divider, buzz_divider)
    ) |> Enum.join(" ")
  end
end
...
defmodule Test do
  ...
  test "should calculate fizzbuzz serie" do
    first_case = Fizzbuzz.get_serie(3, 5, 10);
    second_case = Fizzbuzz.get_serie(2, 7, 15);

    assert first_case == "1 2 F 4 B F 7 8 F B"
    assert second_case == "1 F 3 F 5 F B F 9 F 11 F 13 FB 15"
  end
end
